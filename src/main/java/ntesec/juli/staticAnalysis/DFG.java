...
public class DFG {
	...
	public static Set<Integer> analyzeIfDD(Set<Integer> baseSet, int if_line,
			String var) {
	...
	public static Set<Integer> analyzeWhileDD(Set<Integer> baseSet,
			int while_line, String var) {
		for (int base : baseSet) // 求 IF、ELSIF、ELSE 的依赖
		{
	...
	public static List<Integer> analyzeTriggerStruct(int trigger_line,
			int current) {
	...
	/**
	 * 数据依赖实现算法: @ base : 是表示变量当前所定位的行号 @ current : 是表示当前待判定的节点号 @ var :
	 * 是表示当前求解的变量
	 */

	public static Set<Integer> analyzeDD(Set<Integer> baseSet, int current,
			String var) {
...
//            System.out.println("current Label: " + DFGraphList.get(index).label);
			...
			if (statement.label.equals("VarDefine")
				...
				for (int base : baseSet) {
					// ----------
					// System.out.println(DFGraphList.get(index).REF.toString()
					// + " 包含 " + var + " ?");
					// System.out.println("var >>> " +
					// statement.REF.contains(var));
					// -----------
...
//                System.out.println("listTrigger == " + listTrigger.toString());
	...
	public List<Statement> getDFGbaseCFGandVars(List<Statement> cfgList,
			Set<String> vars) {
		// 将 DFGraphList 初始化
