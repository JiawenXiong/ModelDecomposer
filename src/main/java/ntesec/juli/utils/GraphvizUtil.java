...
public class GraphvizUtil {
	...
	public static void transformSDGToDot(Map<String, List<Statement>> mapSDG,
			Map<String, Set<Integer>> mapInters) {
...
//        graphvizCode.append("size = \"15, 8\"");
			...
			for (int i = 0; i < graphList.size(); ++i) {
				...
				for (int k : CFGNexts) {
					...
					graphvizCode.append("\n\"Statement " + Integer.toString(i)
							+ ": " + graphList.get(i).sliceCode
							+ "\"->\"Statement " + Integer.toString(k) + ": "
							+ graphList.get(k).sliceCode + "\"");
				}
			}
			for (int i = 0; i < graphList.size(); ++i) {
				...
				for (int k : DFGNexts) {
					graphvizCode.append("\n\"Statement " + Integer.toString(i)
							+ ": " + graphList.get(i).sliceCode
							+ "\"->\"Statement " + Integer.toString(k) + ": "
							+ graphList.get(k).sliceCode + "\"[style=dotted]");
				}
			}
		}
		...
		for (String keyname : mapInters.keySet()) {
			...
			for (int i : inters) {
				...
				String[] tmp = fromCode.split(":="); // eg: (x, y :=)? check(a,
														// 1);
				if (tmp.length == 1) // eg: check(a, 1);
				{
					// fromDEF 为空， fromREF为括号里内容
					String[] fromSplit = tmp[0]
							.substring(tmp[0].indexOf("(") + 1,
									tmp[0].indexOf(")"))
							.split(",");
				...
				{
					// fromREF为括号里内容
					String[] fromREFsplit = tmp[1]
							.substring(tmp[1].indexOf("(") + 1,
									tmp[1].indexOf(")"))
							.split(",");
				...
				String[] toREFsplit = toCode
						.substring(toCode.indexOf(":") + 1, toCode.indexOf(")"))
						.split(",");
				...
				if (graphListTo.get(Statement).label.equals("RETURN")) {
					...
					System.out.println("含有Return的程序: "
							+ graphListTo.get(0).sliceCode + " - "
							+ graphListTo.get(Statement).sliceCode);
				}

				// 连接两者的子点
				System.out.println("程序间关系 REF：" + fromREF.toString() + " - "
						+ toREF.toString());
				System.out.println("程序间关系 DEF：" + fromDEF.toString() + " - "
						+ toDEF.toString());
				if (fromREF.size() == toREF.size()
						&& fromDEF.size() == toDEF.size()) {
				...
				// 绘制 from -> to
				System.out.println(
						"from - to " + fromREF.size() + " - " + toREF.size());
				for (int k = 0; k < fromREF.size(); ++k) {
					graphvizCode.append("\n\"Statement " + Integer.toString(i)
							+ " [Out]: " + fromREF.get(k) + "\"->\"Statement "
							+ Integer.toString(0) + " [In]: " + toREF.get(k)
							+ "\"[style=dotted]");
				}

				System.out.println(
						"from - to " + fromDEF.size() + " - " + toDEF.size());
				for (int k = 0; k < fromDEF.size(); ++k) {
					graphvizCode.append("\n\"Statement " + Integer.toString(0)
							+ " [Out]: " + toDEF.get(k) + "\"->\"Statement "
							+ Integer.toString(i) + " [In]: " + fromDEF.get(k)
							+ "\"[style=dotted]");
				}
		...
		FileUtil.writeFile(graphvizCode.toString(), ("resources/dot/SDG.dot"));

	}

	public static void transformPDGToDot(List<Statement> graphList,
			String processname) {
		StringBuffer graphvizCode = new StringBuffer();
		graphvizCode.append("digraph G");
		graphvizCode.append("\n{");
		...
		for (int i = 0; i < graphList.size(); ++i) {
			...
			for (int k : CFGNexts) {
				...
				graphvizCode.append("\n\"Statement " + Integer.toString(i)
						+ ": " + graphList.get(i).sliceCode + "\"->\"Statement "
						+ Integer.toString(k) + ": "
						+ graphList.get(k).sliceCode + "\"");
			}
		}
		...
		FileUtil.writeFile(graphvizCode.toString(),
				("resources/dot/" + processname + ".dot"));
	}
...
/**
 * 
 * dot -Tpng sorting.dot -o sorting.png open sorting.png
 * 
 * dot -Tpng checkProduct.dot -o checkProduct.png open checkProduct.png
 * 
 * dot -Tpng SDG.dot -o SDG.png open SDG.png
 * 
 */