...
public class Cooperation {
	...
	public static Map<String, List<DecomposeStruct>> cooperationProgram(
			Map<String, List<DecomposeStruct>> mapDecompose,
			Map<String, List<Statement>> mapDFGs, String mainProgram,
		...
		for (String baseCU : mapDecompose.keySet()) // 当前待处理的CU
		{
			...
			for (int i = 0; i < listBaseCU.size(); ++i) // baseCU当前待验证的行代码：判断该行代码是否由向前的数据依赖
			{
				...
				if (baseLineInSDG < 0) // 小于0表示非原始代码，而是添加的TRIGGER(TRUE),
										// CHANNEL等
				{
					continue;
				}
				...
				if (DFGbefores.size() > 0) {
					...
					for (int before : DFGbefores) {
						...
						for (String otherCU : mapDecompose.keySet()) // 待做匹配查找的cu,
																		// 即在otherCU代码中查找before所表示的依赖
						{
							List<DecomposeStruct> listOtherCU = mapDecompose
									.get(otherCU);

							for (int j = 0; j < listOtherCU.size(); ++j) {
								if (before == listOtherCU.get(j).lineInSDG) // baseCU中第i
																			// 行代码
																			// 和
																			// otherCU中第j行代码有数据依赖。
								{
									...
									// 判断 i、j
									// 是否在mapDecompose里的同一个cu的同一trigger中。(判断方法：向上找到第一个level为1的就是所属trigger)
									if (baseCU.equals(otherCU)) // 在分解代码的同一个cu的同一个trigger中
									{
										if (whetherInSameDecomposeTrigger(
												listBaseCU, listOtherCU, i,
												j)) {
											continue;
										}
									...
									String synVar = getSyncVar(listDD,
									...
									if (whetherInSameOriginalTrigger(listDD,
										...
										if (type == 0) {
											System.out.println(
													"before in DFGBefores : "
															+ before);
											listBaseCU.add(i,
													new DecomposeStruct(-3,
															listBaseCU.get(
																	i).level,
															"CHANNEL.DD.?"
																	+ Integer
...
//                                    }
										...
										listBaseCU.add(i, new DecomposeStruct(
												-4, listBaseCU.get(i).level,
												"SYNC.DATA." + Integer.toString(
														channelDDMessage) + ":"
														+ synVar + ";"));
										++i;
										isSYNCInOtherCU = true;
	...
	public static boolean whetherInSameOriginalTrigger(List<Statement> listDD,
			int i, int j) {
		int index1 = -1;
		for (int k = i; k >= 0; --k) {
	...
	public static boolean whetherInSameDecomposeTrigger(
			List<DecomposeStruct> listbaseCU, List<DecomposeStruct> listOtherCU,
			int i, int j) {
		int index1 = -1;
		for (int k = i; k >= 0; --k) {
	...
	public static int getChannelPlaceType(List<Statement> listDD,
			int baseLineInSDG, int before) {
		...
		System.out.println(
				"listUpBaseCULines   = " + listUpBaseCULines.toString());
		System.out.println(
				"listUpBeforeCULines = " + listUpBeforeCULines.toString());
...
//                    bothParentLine = listUpBeforeCU.get(j);
		...
		for (int m = 0; m < bothParentLineIndex; ++m) {
			...
			if (listDD.get(k).label.equals("IF")
					|| listDD.get(k).label.equals("ELSIF")
					|| listDD.get(k).label.equals("ELSE")) {
				return 1;
	...
	public static String getSyncVar(List<Statement> listDD, int baseLineInSDG,
			int before) {
