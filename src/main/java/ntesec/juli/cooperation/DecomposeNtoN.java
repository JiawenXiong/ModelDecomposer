package ntesec.juli.cooperation;
...
public class DecomposeNtoN {
	...
	public static void print(Map<String, List<DecomposeStruct>> mapDecompose) {
		for (String key : mapDecompose.keySet()) {
			...
			for (DecomposeStruct decompose : decomposeStructList) {
				...
				System.out.print("line:" + decompose.lineInSDG + "\tlevel:"
						+ decompose.level + "\tcode:");
	...
	public static List<String> outCode2Gen(
			Map<String, List<DecomposeStruct>> mapDecompose) // 输出带花括号的标准子模型代码
																// (层级多的时候新问题...)
	{
		...
		for (String key : mapDecompose.keySet()) {
			...
			Stack<Integer> levelStack = new Stack<>(); // levelStack
														// 记录目前缩进的累计级别个数，相当于入栈
			...
			for (DecomposeStruct decompose : decomposeStructList) {
				...
				if (currentLevel < levelStack.peek()) {
					while (currentLevel < levelStack.peek()) {
						...
						// System.out.print("line:右\tlevel:" + levelStack.peek()
						// + "\tcode:");
					...
					// System.out.print("line:左\tlevel:" + levelStack.peek() +
					// "\tcode:");
					System.out.println(kong + "{");
					codesInKey += (kong + "{");
	...
	// 为下一个算法运行做环境计算准备
	public void decomposeModelNtoN(AST ast,
			Map<String, List<Statement>> mapDFGs) {
		this.channelCDMessage = 1;
		this.ast = ast;
		...
		/**
		 * constraint : A { PATHSET, pathSensor}; constraint : B { SREAD1,
		 * SWRITE1, sensor1}; constraint : C { SREAD2, SWRITE2, sensor2};
		 * constraint : D { SORTSET, SCANNER, sortSensor};
		 */

		// 判断给定constraint是否包括所有资源信息，即: ast.constraint |= (ast.SensorsSet,
		// ast.DeviceSet)

		Map<String, Set<String>> constraintMap = AST.constraintMap; // 从ast.constraintMap
																	// 提取所有的资源
																	// constraintSet
		...
		Map<String, Set<String>> mapResToCUs = new HashMap<>(); // 每个资源可以被使用的cu集合。eg..
																// <'Sensor1',
																// {A, B, C}>
		...
		System.out.println(
				"mapResToCUs.keySet()：" + mapResToCUs.keySet().toString());
		for (String res : mapResToCUs.keySet()) {
			Set<String> setCUs = mapResToCUs.get(res); // 每个资源对应多个CU

			System.out
					.println("res " + res + "\t:setCUs = " + setCUs.toString());
...
//        }
	...
	/**
	 * mapDFGs: 系统控制图 mainProgram: 名字 resToCU: 例如：resToCU.put("sensor2", "B");
	 * capacityCU: 例如：["A","B"]
	 */
	public static Map<String, List<DecomposeStruct>> mapDecompose = new HashMap<>(); // 存放单个方案的结果

	public static Map<String, List<DecomposeStruct>> algorithmNtoN(
			Map<String, String> resToCU, String mainProgram) {
		// N-N 算法
		// 根据资源约束，对main程序进行初步插桩
		List<Statement> mainGraphList = new ArrayList<>(
				mapDFGs.get(mainProgram));
		...
		for (int i = 0; i < mainGraphList.size(); ++i) {
			...
			for (String res : resToCU.keySet()) {
				if (mainGraphList.get(i).REF.contains(res)
						|| mainGraphList.get(i).DEF.contains(res)) {
...
//		System.out.println("数据共享协作通信个数：" + (cooperation.syncMessage - 1));
	...
	public static Map<String, List<DecomposeStruct>> decomposeProgramNtoN(
			int current, Map<String, List<DecomposeStruct>> mapDecompose,
			List<String> pileList) {
		...
		for (int i = 0; i < currentNexts.size(); ++i) {
			...
			if (statement.label.equals("VarDefine")
					|| statement.label.equals("AssignVariable")
					|| statement.label.equals("AssignFunction")
					|| statement.label.equals("AssignInvoke")
					|| statement.label.equals("RETURN")
					|| statement.label.equals("STOP")) {

				String assignPileCU = pileList.get(index);
				subMapDecompose.get(assignPileCU)
						.add(new DecomposeStruct(index,
								mainGraphList.get(index).level,
								mainGraphList.get(index).sliceCode));
				...
				for (int subIf : listIf) {
					...
					subMapDecompose.get(subIfPileCU)
							.add(new DecomposeStruct(index,
									mainGraphList.get(subIf).level,
									mainGraphList.get(subIf).sliceCode));

					Map<String, List<DecomposeStruct>> subIfDecompose = decomposeProgramNtoN(
							subIf, mapDecompose, pileList);
					for (String cu : subIfDecompose.keySet()) {
						...
						if (cu.equals(subIfPileCU)) {
							subMapDecompose.get(cu)
									.addAll(subIfDecompose.get(cu));
						} else {

							subMapDecompose.get(subIfPileCU)
									.add(new DecomposeStruct(-2,
											mainGraphList.get(subIf).level + 1,
											"CHANNEL.CD.!"
													+ Integer.toString(
															channelCDMessage)
													+ ";"));
...
//                            mapDecompose.get(cu).addAll(2, subIfDecompose.get(cu));
							...
							mapDecompose.get(cu).add(0,
									new DecomposeStruct(-2, 1,
											"TRIGGER(CHANNEL.CD.?"
													+ Integer.toString(
															channelCDMessage)
													+ ")"));

							mapDecompose.get(cu).addAll(1,
									subIfDecompose.get(cu));
							...
							mapDecompose.get(cu).add(
									subIfDecompose.get(cu).size() + 1, // TODO:
																		// ""subIfDecompose.get(cu).size()
																		// +
																		// 1""可能有问题
									new DecomposeStruct(-2, 2,
											"CHANNEL.CD.!" + Integer.toString(
													channelCDMessage)));
							channelCDMessage++;
						}
					}
				}
			} else if (statement.label.equals("ELSIF")
					|| statement.label.equals("ELSE")) {
				continue;
			} else if (statement.label.equals("TRIGGER")) {
				String triggerPileCU = pileList.get(index); // 获取当前是A，B，C，D......
				mapDecompose.get(triggerPileCU).add(new DecomposeStruct(index,
						1, mainGraphList.get(index).sliceCode));

				Map<String, List<DecomposeStruct>> triggerDecompose = decomposeProgramNtoN(
						index, mapDecompose, pileList);
				...
				for (String cu : triggerDecompose.keySet()) {
					...
					if (cu.equals(triggerPileCU)) {
						subMapDecompose.get(triggerPileCU)
								.addAll(triggerDecompose.get(cu));
					} else {
...
//                        mapDecompose.get(cu).addAll(2, triggerDecompose.get(cu));
						...
						mapDecompose.get(cu).add(0, new DecomposeStruct(-2, 1,
								"TRIGGER(CHANNEL.CD.?"
										+ Integer.toString(channelCDMessage)
										+ ")"));

						mapDecompose.get(cu).addAll(1,
								triggerDecompose.get(cu));
						channelCDMessage++;
...
//                channelCDMessage++;
				mapDecompose.get(triggerPileCU)
						.addAll(subMapDecompose.get(triggerPileCU));
	...
	public static List<Integer> computeIfStruct(List<Statement> mainGraphList,
			int if_line) {
	...
	/**
	 * - 根据控制流向后插桩 var 表示当前节点所分配的处理器 current 表示当前节点 pileMainList 表示当前程序的插桩标记
	 * mainGraphList 表示当前进程的图
	 */
	public static void pileSD(int current, List<String> pileMainList,
			List<Statement> mainGraphList) {
		List<Integer> currentNexts = mainGraphList.get(current).CFGNexts;
